...
export const getTerms = async (term_uid: string) => {
  const response = (await Stack.getTerms(term_uid)) as any;
  liveEdit && addEditableTags(response[0], "blog_post", true);
  return response;
};


export const getEntriesByTerm = async (term_uid: any) => {
  const response = (await Stack.getEntriesByTerm(term_uid)) as any;
  liveEdit && addEditableTags(response[0], "blog_post", true);
  return response;
};
